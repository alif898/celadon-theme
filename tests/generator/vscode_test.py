...
def test_vscode_generator_files(mock_palette: PaletteModel, mock_config: ConfigModel, mock_env: Environment, temp_dist_path: Path) -> None:
    generator = VsCodeGenerator(mock_palette, mock_config, mock_env, dist_path=temp_dist_path)
    
    generator.generate_theme_files()
    
    themes_path = temp_dist_path / "themes"
    assert (themes_path / "celadon-theme-color-theme.json").exists()
    assert (themes_path / "celadon-theme-color-theme.json").read_text() == "THEME: Test Theme"

def test_vscode_generator_metadata(mock_palette: PaletteModel, mock_config: ConfigModel, mock_env: Environment, temp_dist_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    svg_file.write_text('<svg width="10" height="10"><rect width="10" height="10" fill="red"/></svg>')
    
    # Mock the paths in celadon_theme.generator.vscode
...
def test_vscode_generator_metadata_no_svg(mock_palette: PaletteModel, mock_config: ConfigModel, mock_env: Environment, temp_dist_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_vscode_generator_readme_prefix(mock_palette: PaletteModel, mock_config: ConfigModel, mock_env: Environment, temp_dist_path: Path) -> None:
